...
class Book_Review_Admin {
	...
	public function enqueue_admin_styles( $hook_suffix ) {
		...
		if ( $hook_suffix == 'edit.php' ) {
			...
			if ( $screen->post_type == 'post' ) {
				wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/posts-admin.css', __FILE__ ), array(), Book_Review::VERSION );
			}
		}
		else if ( $hook_suffix == 'post-new.php' || $hook_suffix == 'post.php' ) {
			wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/meta-box.css', __FILE__ ), array(), Book_Review::VERSION );
		}
		else if ( $hook_suffix == $this->plugin_screen_hook_suffix ) {
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), Book_Review::VERSION );
		}
	}
	...
	public function enqueue_admin_scripts( $hook_suffix ) {
		...
		if ( $hook_suffix == 'post-new.php' || $hook_suffix == 'post.php' ) {
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/meta-box.js', __FILE__ ), array( 'jquery' ), Book_Review::VERSION );
			wp_enqueue_script( 'jquery-ui-spinner' );
		}
		else if ( $hook_suffix == $this->plugin_screen_hook_suffix ) {
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/admin.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), Book_Review::VERSION );
		}
	}
	...
	public function display_plugin_admin_page() {
		...
		// Tooltip
		$tooltip = '<img src="' . plugins_url('assets/images/tooltip.gif', __FILE__ ) . '" />';
	...
	/**
	 * Create the "Release Date Format" dropdown.
	 *
	 * @since     2.0
	 */
	public function create_date_format_field() {
		$formats = array(
			"none" => "None",
		...
		echo '<select id="book_review_date_format" name="book_review_general[book_review_date_format]">';

		foreach( $formats as $type => $format ) {
			$selected = ( $options['book_review_date_format'] == $type ) ? 'selected="selected"' : '';
			echo '<option value="' . $type . '" '. $selected . '>' . $format . '</option>';
		}
	...
	/**
	 * Create a field for the rating image URL.
	 *
	 * @since     1.0
	 */
	...
	/**
	 * Create the "Number of Links" dropdown.
	 *
	 * @since     1.0
	 */
	...
	/**
	 * Create the "Open links in new tab" checkbox.
	 *
	 * @since     1.0
	 */
	...
	/**
	 * Create a field for the "Link Text".
	 *
	 * @since     1.0
	 */
	public function create_link_text_field( $args ) {
		...
		echo '<input id="' . $args['id'] .'" type="text" name="book_review_links[' . $args['id'] . ']" class="text-input" value="' . $value . '" />';
	}

	/**
	 * Create a field for the "Link Image URL".
	 *
	 * @since     1.0
	 */
	public function create_link_image_field( $args ) {
		...
		echo '<input id="' . $args['id'] .'" type="text" name="book_review_links[' . $args['id'] . ']" class="text-input" value="' . esc_url( $value ) . '" />';
	}
		...
		if ( $column == 'rating' ) {
			...
			if ( isset( $values["book_review_rating"] ) != null ) {
				...
				if ( !empty( $rating ) && ( $rating != "-1" ) ) {
					echo '<img src="' . $plugin->get_rating_image( $rating ) . '" class="book_review_column_rating" />';
				}
			}
		}
	}
	...
	public function get_book_info() {
		if ( wp_verify_nonce( $_REQUEST['nonce'], 'ajax_isbn_nonce' ) ) {
			...
			if ( isset( $api_key) && !empty( $api_key ) ) {
				$url = 'https://www.googleapis.com/books/v1/volumes?q=isbn:' . $_POST['isbn'] . '&key=' . $api_key;
	...
	public function render_meta_box( $post ) {
		...
		$isbn = isset( $values['book_review_isbn'] ) ? $values['book_review_isbn'][0] : '';
	...
	public function render_links() {
		...
		for ( $i = 1; $i <= 5; $i++ ) {
			if ( isset( $links['book_review_link_text' . $i] ) && ( $num_links >= $i ) ) { ?>
	...
	public function save_meta_box( $post_id ) {
		...
		if ( !isset( $_POST['book-review-meta-box-nonce'] ) || !wp_verify_nonce( $_POST['book-review-meta-box-nonce'], 'save_meta_box_nonce' ) )
			return;
